...
const elBtnLayout = document.getElementById('btn-layout');
...
let wideTab = 'wrapped'; // current tab in wide mode
let wideLbTimer = null;

function setWideMode(enabled, save = true) {
  isWideMode = enabled;
  document.body.classList.toggle('wide-mode', enabled);
  elBtnLayout.textContent = enabled ? '▣' : '⊞';
  elBtnLayout.title = enabled ? 'Switch to compact layout' : 'Switch to wide layout';
  if (enabled) {
    showWideTab(wideTab);
    loadWideSidebar();
    if (!wideLbTimer) wideLbTimer = setInterval(loadWideSidebar, 30000);
    // Sync now-playing bar with current track
    if (liveTrack) updateWideNowPlaying(liveTrack);
  } else {
    if (wideLbTimer) { clearInterval(wideLbTimer); wideLbTimer = null; }
  }
  if (save) window.musicToDiscord.setSetting('wideMode', enabled);
}

function showWideTab(tab) {
  wideTab = tab;
  // Update tab buttons
  ['wrapped','recs','share','settings','dev'].forEach(t => {
    const btn = document.getElementById(`wtab-btn-${t}`);
    if (btn) btn.classList.toggle('active', t === tab);
  });

  // Render content into #wide-content
  const content = document.getElementById('wide-content');
  if (!content) return;

  if (tab === 'wrapped') {
    // Clone the wrapped view into wide-content
    const src = document.getElementById('view-wrapped');
    content.innerHTML = '';
    content.appendChild(src.cloneNode(true));
    // Re-run wrapped load (needs fresh DOM refs from the clone)
    loadWrappedInto(content.querySelector('#view-wrapped'));
  } else if (tab === 'recs') {
    const src = document.getElementById('view-recs');
    content.innerHTML = src.outerHTML;
    loadRecsInto(content.querySelector('#view-recs'));
  } else if (tab === 'share') {
    const src = document.getElementById('view-share');
    content.innerHTML = src.outerHTML;
    loadShareInto(content.querySelector('#view-share'));
  } else if (tab === 'settings') {
    // Show settings panel inline
    const settingsEl = document.getElementById('settings');
    content.innerHTML = `<div style="padding:16px;">${settingsEl.innerHTML}</div>`;
    // Re-wire background customizer into the clone
    rebuildBgPresetsIn(content);
  } else if (tab === 'dev') {
    const src = document.getElementById('view-dev');
    content.innerHTML = src.outerHTML;
    refreshDevPanel();
  }
}

// Wide now-playing bar updates
function updateWideNowPlaying(track) {
  const nameEl = document.getElementById('wide-track-name');
  const artistEl = document.getElementById('wide-track-artist');
  const artImg = document.getElementById('wide-art-img');
  const artFallback = document.getElementById('wide-art-fallback');
  const timerEl = document.getElementById('wide-timer');

  if (!nameEl) return;

  if (!track || track.state === 'not_running' || track.state === 'stopped') {
    nameEl.textContent = 'Nothing playing';
    artistEl.textContent = 'Open Apple Music and press play';
    if (artImg) { artImg.style.display = 'none'; }
    if (artFallback) artFallback.style.display = '';
    if (timerEl) { timerEl.textContent = '0:00'; timerEl.classList.add('paused'); }
    document.getElementById('wide-groove-fill').style.width = '0%';
    document.getElementById('wide-groove-needle').style.left = '0%';
    document.getElementById('wide-time-elapsed').textContent = '0:00';
    document.getElementById('wide-time-remaining').textContent = '−0:00';
    return;
  }
  ...
  if (track.artworkDataUrl && artImg) {
    artImg.src = track.artworkDataUrl;
    artImg.style.display = 'block';
  ...
  if (timerEl) timerEl.classList.toggle('paused', track.state !== 'playing');
}

// Paint wide progress bar (mirrors the narrow paintTime but for wide bar)
function paintWideTime() {
  if (!liveTrack) return;
  const isPlaying = liveTrack.state === 'playing';
  const elapsed = isPlaying ? (Date.now() - liveAnchorMs) / 1000 : 0;
  const position = Math.min(liveTrack.duration || 0, (liveTrack.position || 0) + elapsed);
  const duration = liveTrack.duration || 0;
  const pct = duration > 0 ? Math.min(100, (position / duration) * 100) : 0;
  const remaining = Math.max(0, duration - position);

  const fill = document.getElementById('wide-groove-fill');
  const needle = document.getElementById('wide-groove-needle');
  const elapsed2 = document.getElementById('wide-time-elapsed');
  const rem = document.getElementById('wide-time-remaining');
  const timer = document.getElementById('wide-timer');

  if (fill) fill.style.width = `${pct}%`;
  if (needle) needle.style.left = `${pct}%`;
  if (elapsed2) elapsed2.textContent = formatTime(position);
  if (rem) rem.textContent = `−${formatTime(remaining)}`;
  if (timer) timer.textContent = formatTime(remaining);
}

setInterval(paintWideTime, 1000);

// Hook wide bar updates into existing state updates
const _origReanchor = reanchor;
// Patch: after reanchor, also update wide bar
const origPaintTime = paintTime;
function paintTimeAndWide() {
  origPaintTime();
  if (isWideMode) paintWideTime();
}

// Wide sidebar leaderboard
async function loadWideSidebar() {
  const listEl = document.getElementById('wide-leaderboard-list');
  const emptyEl = document.getElementById('wide-leaderboard-empty');
  const errorEl = document.getElementById('wide-leaderboard-error');
  if (!listEl) return;

  const entries = await window.musicToDiscord.getLeaderboard().catch(() => null);
  ...
  listEl.innerHTML = entries.slice(0, 20).map((e, i) => {
    const rank = i + 1;
    const rankClass = rank === 1 ? 'gold' : rank === 2 ? 'silver' : rank === 3 ? 'bronze' : '';
    const rankLabel = rank <= 3 ? ['🥇','🥈','🥉'][rank-1] : rank;
    const h = Math.floor(e.totalSeconds / 3600);
    const m = Math.floor((e.totalSeconds % 3600) / 60);
    const timeStr = h > 0 ? `${h}h ${m}m` : `${m}m`;
    return `<div class="wide-lb-row">
      <div class="wide-lb-rank ${rankClass}">${rankLabel}</div>
      <div class="wide-lb-name">${esc(e.username)}</div>
      <div class="wide-lb-time">${timeStr}</div>
    </div>`;
  }).join('');
}

// Helpers to load views into wide-content clones
async function loadWrappedInto(container) {
  if (!container) return;
  container.style.display = 'flex';
  const months = await window.musicToDiscord.getWrappedMonths();
  const select = container.querySelector('#wrapped-month-select');
  const emptyEl = container.querySelector('#wrapped-empty');
  const contentEl = container.querySelector('#wrapped-content');
  if (!months || months.length === 0) {
  ...
  if (select) {
    ...
    select.onchange = () => renderWrappedInto(select.value, container);
  }
  renderWrappedInto(months[0], container);
  // Also load streaks
  const streaks = await window.musicToDiscord.getStreaks();
  if (streaks) {
    const sc = container.querySelector('#streak-current');
    const sl = container.querySelector('#streak-longest');
    const st = container.querySelector('#streak-today');
    if (sc) sc.textContent = streaks.current > 0 ? `${streaks.current}d` : '0';
    if (sl) sl.textContent = streaks.longest > 0 ? `${streaks.longest}d` : '0';
    if (st) st.textContent = streaks.todayCount;
  }
}

async function renderWrappedInto(monthKey, container) {
  const data = await window.musicToDiscord.getWrapped(monthKey);
  const emptyEl = container.querySelector('#wrapped-empty');
  const contentEl = container.querySelector('#wrapped-content');
  if (!data) {
  ...
  if (q('wrapped-total-plays')) q('wrapped-total-plays').textContent = data.totalPlays.toLocaleString();
  if (q('wrapped-total-time')) q('wrapped-total-time').textContent = formatListeningTimeWrapped(data.totalSeconds);
  if (q('wrapped-top-artist-short')) q('wrapped-top-artist-short').textContent = data.topArtists[0] ? data.topArtists[0].artist.split(' ')[0] : '—';
  if (q('wrapped-busiest-hour')) q('wrapped-busiest-hour').textContent = formatHour(data.busiestHour);
  const songsEl = q('wrapped-top-songs');
  if (songsEl) songsEl.innerHTML = data.topSongs.map((s, i) => `<div class="wrapped-rank-row"><div class="wrapped-rank-num">${i+1}</div><div class="wrapped-rank-info"><div class="wrapped-rank-name">${esc(s.name)}</div><div class="wrapped-rank-sub">${esc(s.artist)}</div></div><div class="wrapped-rank-count">${s.count}×</div></div>`).join('');
  const artistsEl = q('wrapped-top-artists');
  if (artistsEl) artistsEl.innerHTML = data.topArtists.map((a, i) => `<div class="wrapped-rank-row"><div class="wrapped-rank-num">${i+1}</div><div class="wrapped-rank-info"><div class="wrapped-rank-name">${esc(a.artist||'Unknown')}</div></div><div class="wrapped-rank-count">${a.count}×</div></div>`).join('');
}

async function loadRecsInto(container) {
  if (!container) return;
  container.style.display = 'flex';
  const recs = await window.musicToDiscord.getRecommendations().catch(() => []);
  const listEl = container.querySelector('#recs-list');
  const emptyEl = container.querySelector('#recs-empty');
  if (!recs || recs.length === 0) {
    if (emptyEl) emptyEl.style.display = 'block';
  ...
  if (emptyEl) emptyEl.style.display = 'none';
  if (listEl) listEl.innerHTML = recs.map(r => `<div class="rec-row"><div class="rec-icon">🎵</div><div class="rec-info"><div class="rec-name">${esc(r.name)}</div><div class="rec-artist">${esc(r.artist)}</div></div></div>`).join('');
}

async function loadShareInto(container) {
  if (!container) return;
  container.style.display = 'flex';
  const data = await window.musicToDiscord.getShareCard().catch(() => null);
  const emptyEl = container.querySelector('#share-empty');
  const cardEl = container.querySelector('#share-card');
  ...
  if (copyBtn) {
    ...
    copyBtn.onclick = async () => {
      const text = `🎵 Listening to: ${data.name} by ${data.artist}${data.album ? ' (' + data.album + ')' : ''} — via MusicToDiscord`;
  ...
  const q = (id) => container.querySelector(`#${id}`);
  if (q('share-card-label')) q('share-card-label').textContent = data.state === 'paused' ? 'Paused' : 'Now Playing';
  if (q('share-card-name')) q('share-card-name').textContent = data.name;
  if (q('share-card-artist')) q('share-card-artist').textContent = data.artist;
  if (q('share-card-album')) q('share-card-album').textContent = data.album || '';
}

function rebuildBgPresetsIn(container) {
  const presetsEl = container.querySelector('#bg-presets');
  if (!presetsEl) return;
  presetsEl.innerHTML = BG_PRESETS.map(p => `<div class="bg-preset" data-value="${p.value}" style="background: ${p.value}" title="${p.label}"></div>`).join('');
  presetsEl.querySelectorAll('.bg-preset').forEach(el => {
    el.classList.toggle('active', el.dataset.value === currentBg);
    el.addEventListener('click', () => saveBg(el.dataset.value));
  });
  const input = container.querySelector('#bg-custom-input');
  if (input) {
    input.value = currentBg === '#0F0F14' ? '' : currentBg;
    input.addEventListener('keydown', e => { if (e.key === 'Enter' && input.value.trim()) saveBg(input.value.trim()); });
  }
  const resetBtn = container.querySelector('#bg-reset-btn');
  if (resetBtn) resetBtn.addEventListener('click', () => saveBg('#0F0F14'));
}

// Wide tab click handlers
['wrapped','recs','share','settings','dev'].forEach(tab => {
  const btn = document.getElementById(`wtab-btn-${tab}`);
  if (btn) btn.addEventListener('click', () => showWideTab(tab));
});

// Wide settings button (gear icon in now-playing bar)
const wideSettingsBtn = document.getElementById('wide-settings-btn');
if (wideSettingsBtn) wideSettingsBtn.addEventListener('click', () => showWideTab('settings'));

// Layout toggle button
elBtnLayout.addEventListener('click', () => setWideMode(!isWideMode));

// Hook state updates into wide now-playing bar
const _origRender = render;
window.musicToDiscord.onStateUpdate((state) => {
  _origRender(state);
  if (isWideMode && state.track) updateWideNowPlaying(state.track);
  // Show dev tab in wide mode if unlocked
  if (state.devMode) {
    const devBtn = document.getElementById('wtab-btn-dev');
    if (devBtn) devBtn.style.display = '';
  }
});

// Load saved layout mode on startup
window.musicToDiscord.getSettings().then(settings => {
  if (settings && settings.wideMode) setWideMode(true, false);
});
